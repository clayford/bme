...
t43 <- matrix(c(12,2,7,9),ncol=2, dimnames = list(diarrhea = c("yes","no"), antibody=c("low","high")))
t43
addmargins(t43)
...
library(epitools)
or.out <- oddsratio(t(t43), rev="both", method="wald", verbose = T)
or.out
...
chi_p <- sum(((t43 - expected(t43))^2)/expected(t43))
pchisq(chi_p, df = 1, lower.tail = FALSE)
# or do this
chisq.test(t43, correct = F)

# Wald
chi_w <- (log(or.out$measure[2,1])^2)*(1/(sum(1/expected(t43))))
pchisq(chi_w, df = 1, lower.tail = FALSE)

# likelihood ratio
chi_lr <- 2*sum(t43*log(t43/expected(t43)))
pchisq(chi_lr, df = 1, lower.tail = FALSE)

# example 4.2
t45 <- matrix(c(23,25,31,113),ncol=2, dimnames = list(survival = c("dead","alive"), 
                                                      receptor.level=c("low","high")))
t45
oddsratio(t(t45), re="both", method = "wald", verbose=TRUE)
...
t45b <- cbind(low, high)
oddsratio(t(t45b), rev = "both", method = "wald")
...
# example 4.4
oddsratio(t(t43), rev="both", method = "fisher")
fisher.test(t43)
...
# example 4.5
oddsratio(t(t45), rev = "both", method = "fisher")
...
MH <- ((12 - expected(t43)[1])^2)/((14*16*11*19)/(30^2*29))
